    ...
    internal class Buildings : Construction_Item
    {
        ...
        public List<int> GetMaxStoredResourceCounts(int level)
        {
           /* var maxStoredResourceCounts = new List<int>();
            ...
            var propertyName = "MaxStoredResource";
            if (GetType().GetProperty(propertyName) != null)
            {

                var obj = GetType().GetProperty(propertyName).GetValue(this, null);
                foreach (string t in ((string[]) obj)[level].Split(','))
                {
                    Console.WriteLine(t);
        ...
        public override int GetConstructionTime(int level)
        {
            ...
            if (BuildTimeD.Length > level + 2)
                Total_Time += BuildTimeD[level + 1] * 86400;
            if (BuildTimeH.Length > level + 2)
                Total_Time += BuildTimeH[level + 1] * 3600;
            if (BuildTimeM.Length > level + 2)
                Total_Time += BuildTimeM[level + 1] * 60;
            if (BuildTimeS.Length > level + 2)
                Total_Time += BuildTimeS[level + 1];

            return Total_Time;
           //return BuildTimeS[level] + BuildTimeM[level] * 60 + BuildTimeH[level] * 60 * 60 + BuildTimeD[level] * 60 * 60 * 24;
        }

        public override bool IsHQ() => BuildingClass == "Town Hall";
